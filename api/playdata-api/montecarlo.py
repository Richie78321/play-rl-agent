from tictactoe.agent import QLearningAgent, RandomAgent, Agent
from pathlib import Path
from tictactoe.evaluate import evaluate_game, StepData
    ...
    for _ in tqdm(range(episodes), desc="Generating data"):
        ...
        for x in range(1, len(episode_data), 2):
            ...
            if x == len(episode_data) - 1:
                ...
                playdata.send(process_playdata_json({
                    "initial_state": episode_data[x].state.text_board.flatten(),
                    "action": Board(np_board=episode_data[x].action._board, agent_is_x=False).text_board.flatten(),
                    "resultant_state": episode_data[x].resultant_state.text_board.flatten(),
                ...
                playdata.send(process_playdata_json({
                    "initial_state": episode_data[x].state.text_board.flatten(),
                    "action": Board(np_board=episode_data[x].action._board, agent_is_x=False).text_board.flatten(),
                    "resultant_state": episode_data[x + 1].resultant_state.text_board.flatten(),
                    "reward": episode_data[x].reward - episode_data[x + 1].reward,
                    "agent_is_x": False,
                }))
